...
def process_args(args, defaults):
    ...
    parser.add_argument('--attn-num-layers', dest="attn_num_layers",
                        ...
                        help=(
                        'number of hidden layers in attention decoder cell'
                        ', default=%s'
    ...
    parser.add_argument('--no-load-model', dest='load_model',
                        action='store_false')

    parser.set_defaults(load_model=defaults.LOAD_MODEL)

    parser.add_argument('--old-model-version', dest='old_model_version',
                        action='store_true',
                        help=(
                        'Whether the model was created by old keras version '
                        'or not. Note that we need to make conversions for '
                        'such old models.'
                        ', default=%s' % (defaults.OLD_MODEL_VERSION)))

    parser.add_argument('--no-old-model-version', dest='old_model_version',
                        action='store_false')
    ...
    parser.add_argument('--tb_logs', dest = "tb_logs",
                        type = str, default = defaults.MODEL_DIR,
