...
class CNN(object):
    ...
    def _build_network(self, input_tensor):
        ...
        """
        '''
        Ask John about the late merging and try with valid convs in the
        layers between
        '''
        ...
        model.add(layers.Convolution2D(128, 1, 3, subsample = (1, 1),
                                       border_mode = 'same',
                                       dim_ordering = 'th'))
        model.add(layers.Activation('relu'))
        ...
        model.add(layers.Convolution2D(256, 1, 3, subsample = (1, 1),
                                       border_mode = 'same',
                                       dim_ordering = 'th'))
        ...
        model.add(layers.Convolution2D(512, 1, 3, subsample = (1, 1),
                                       border_mode = 'same',
                                       dim_ordering = 'th'))
        model.add(layers.BatchNormalization(axis = 1))
        model.add(layers.Activation('relu'))
        ...
        model.add(layers.Convolution2D(512, 10, 2, subsample = (1, 1),
                                       border_mode = 'valid',
                                       dim_ordering = 'th'))
        model.add(layers.BatchNormalization(axis = 1))
        model.add(layers.Activation('relu'))
